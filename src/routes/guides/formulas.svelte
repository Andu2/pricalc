<script>
	import { STAT_NAMES, STAT_DISPLAY_NAMES, getTable } from "@src/data";
	import DataComponent from "@src/components/DataComponent.svelte";

	const requiredTables = [ "unit_status_coefficient" ];
	...
	function onDataReady() {
		coefficients = getTable("unit_status_coefficient")[0];
		coefficientStats = [...STAT_NAMES];
	}
</script>
...
<DataComponent requiredTables={requiredTables} onDataReady={onDataReady} >
	...
	<table><tr><th>Stat</th><th>Weight</th></tr>
		{#each coefficientStats as stat}
		<tr><td class="label">{STAT_DISPLAY_NAMES[stat]}</td><td class="number">{coefficients[stat + "_coefficient"]}</td></tr>
		{/each}
		<tr><td class="label">Skill levels</td><td class="number">{coefficients["skill_lv_coefficient"]}</td></tr>
	</table>
	...
	<p>If we analyze this formula, we can see that defense linearly increases the amount of damage a character can take. Defense is like a percentage increase in effective health - adding 3 defense means a character can take additional raw damage equal to 3% of their base health.</p>
	...
	<p>TP Boost and HP Recovery Boost are straight percentage increases. So, TP Boost of 30 means 30% faster TP gain. Note that HP Recovery Boost matters on the healer, not the healee.</p>
</DataComponent>
