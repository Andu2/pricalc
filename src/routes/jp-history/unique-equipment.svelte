<script>
	...
	$: data = getData($enScheduleOffset.uniqueEquipment);
	...
	function getData(offset = $enScheduleOffset.uniqueEquipment) {
...
<JPContentHeader />
<p>This page assumes EN schedule is <strong>{determineOffsetWord($enScheduleOffset.uniqueEquipment)}</strong> by <strong>{Math.abs($enScheduleOffset.uniqueEquipment)}</strong> days.</p>
